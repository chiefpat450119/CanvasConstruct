...
@export var defense: Control
@export var weapon: Control
@export var torso: Control
@export var fight_button: Control
...
func _ready() -> void:
	...
	if _components.is_empty():
		push_warning("SelectionUIAnim has no components to animate.")
		return
...
func _cache_components() -> void:
	...
	_add_component(head, SlideDirection.TOP)
	_add_component(defense, SlideDirection.LEFT)
	_add_component(weapon, SlideDirection.RIGHT)
	_add_component(torso, SlideDirection.BOTTOM)
	_add_component(fight_button, SlideDirection.BOTTOM)
